...
const H = {
  ...
  Accounts: "The AI subscriptions signed in on the router",
  Usage: "How much the router has been used: requests, tokens (the units AI use is counted in) and estimated cost",
  Settings: "Switches for what AI Router adds to Paseo",
  Connection: "Which router this computer talks to, the keys it uses",
...
const expected = {
  ...
  "models tab (combo profiles)": ["Combos as agent profiles", "Show combos as agent profiles", "orchestrating agents read them", "Auto · coding", "auto/coding", "OmniRoute auto combo \"auto/coding\": Quality-first for code. Use it as the model on the AI Router provider", "Auto · best reasoning", "Best discovery (10% explore)", "Team review", "Opus 5.5 first, GPT-6 Sol when Claude is busy. 2 models, priority strategy."],
  ...
  "connection tab (public address ok)": ["Public address", "https://ai-router.example.com", "HTTPS OK · ai-router.example.com", "Your daemons use the endpoint above; people and browsers outside your network use this address.", "Share this router", "Endpoint", "Ask your router admin for your own key", "Claude Code", "export ANTHROPIC_BASE_URL=https://ai-router.example.com", "export ANTHROPIC_AUTH_TOKEN=<your key>", "export CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=1", "export CLAUDE_CODE_DISABLE_FAST_MODE=1", "beta", "Codex", 'base_url = "https://ai-router.example.com/v1"', "Paseo", "paseo plugin install git:https://github.com/itsjustanks/paseo-plugin-ai-router.git:apps/paseo", "export AI_ROUTER_URL=https://ai-router.example.com", "Copied the Claude Code setup", "Dashboard login: ask your router admin."],
